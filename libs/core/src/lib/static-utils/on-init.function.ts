 ...
 * Runs as an effect exactly once and uses the returned function as the cleanup, which will be executuded onDestroy
...
export function onInit(initFn: () => void | EffectCleanupFn) {
  ...
  // the cleanup is then effectively called in the the ngOnDestroy lifecycle.
  effect((onCleanup) => {
