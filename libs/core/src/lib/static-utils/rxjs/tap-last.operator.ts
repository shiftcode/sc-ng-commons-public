 ...
 */
export function tapLast<T>(callback: (val: T) => void): MonoTypeOperatorFunction<T> {
  return function tapLatOperatorFunction(source: Observable<T>): Observable<T> {
...
class TapLastOperator<T> implements Operator<T, T> {
  ...
  call(subscriber: Subscriber<T>, source: any): TeardownLogic {
    return source.subscribe(new TapLastSubscriber(subscriber, this.callback))
  }
}
